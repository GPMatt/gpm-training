...
function autoResponder_run_() {
  ...
  for (var i = 0; i < threads.length; i++) {
    ...
    for (var j = 0; j < messages.length; j++) {
      ...
      // Mark this specific message as read so we don't process it again
      message.markRead();
    }

    // Only archive once every message in the thread has been handled —
    // if something got threaded in that isn't actually a lead reply, leave
    // the thread in the inbox instead of archiving an unread message away.
